...
def ndarray():
    ...
    w = torch.randn(3, 3, 4)  # 每个元素都从均值为0、标准差为1的标准高斯（正态）分布中随机采样。
    print('w:', w)
    ...
    print('cat操作 dim=0', torch.cat((X, Y), dim=0))  #沿行（轴-0，形状的第一个元素）
    print('cat操作 dim=1', torch.cat((X, Y), dim=1))  #按列（轴-1，形状的第二个元素）

    print('X == Y', X == Y)  # 通过 逻辑运算符 构建二元张量
    print('X < Y', X < Y)
    print('张量所有元素的和:', X.sum())  # 张量所有元素的和  产生一个单元素张量
    ...
    print('a:', a)
    print('b:', b)
    print('a + b:', a + b)  # 矩阵a将复制列， 矩阵b将复制行，然后再按元素相加。
    ...
    Z = torch.zeros_like(Y)  #生成全为0，格式为Y的矩阵
    print('id(Z):', id(Z))
    Z[:] = X + Y
    print('id(Z):', id(Z))
